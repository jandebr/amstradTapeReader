...
public class LocomotiveBasicDecompiler implements BasicDecompiler {
	...
	private void init(short[] byteCode) {
	...
	@Override
	public SourceCode decompile(short[] byteCode) {
		init(byteCode);
		...
		try {
			...
			while (announcedLineLengthInBytes > 0) {
				...
				SourceCodeLine line = nextLine(lineNumber, code);
	...
	private SourceCodeLine nextLine(int lineNumber, SourceCode code) {
		StringBuilder line = new StringBuilder(256);
		int bytecodeOffset = byteCodeIndex;
		short b = nextByte();
		while (b != 0x00) {
			SourceCodePosition from = new SourceCodePosition(lineNumber, line.length());
			if (b == 0x01) {
				// statement seperator
				line.append(':');
			...
			SourceCodePosition until = new SourceCodePosition(lineNumber, line.length() - 1);
			if (until.getLinePosition() >= from.getLinePosition()) {
				int bytecodeLength = byteCodeIndex - bytecodeOffset;
				getSourceCodeDecorator().decorate(code, new SourceCodeRange(from, until),
						new ByteCodeRange(bytecodeOffset, bytecodeLength));
				bytecodeOffset = byteCodeIndex;
			}
			b = nextByte();
		}
		return new SourceCodeLine(lineNumber, line);
	}
