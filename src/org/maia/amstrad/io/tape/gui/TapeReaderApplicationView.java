...
public class TapeReaderApplicationView extends JPanel {
	...
	private synchronized void updateResultsViewAsync() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					setResultsView(createResultsView(UIFactory.getScreenSize().width)); // can take time
				} catch (Exception e) {
	...
	private static class ReadingView extends JLabel {
		...
		public ReadingView() {
			super("Reading...", UIResources.spinnerIcon, SwingConstants.CENTER);
			setIconTextGap(8);
		}
